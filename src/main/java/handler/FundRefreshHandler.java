...
public abstract class FundRefreshHandler extends DefaultTableModel {
    ...
    static {
        ...
        if (instance.getValue(WindowUtils.FUND_TABLE_HEADER_KEY) == null) {
            instance.setValue(WindowUtils.FUND_TABLE_HEADER_KEY, "编码,基金名称,估算净值,估算涨跌,更新时间,当日净值,持有价格,收益率");
        }
        String[] configStr = "编码,基金名称,估算净值,估算涨跌,更新时间,当日净值,持有价格,收益率".split(",");
    ...
    private void columnColors(boolean colorful) {
        ...
        DefaultTableCellRenderer cellRenderer2 = new DefaultTableCellRenderer() {
            ...
            public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
                ...
                double temp = 0.0;
                try {
                    String s = value.toString().substring(0,value.toString().indexOf("%"));
                    temp = Double.parseDouble(s);
                } catch (Exception e) {
        ...
        int yieldRateIndex = WindowUtils.getColumnIndexByName(columnNames, "收益率");
        table.getColumn(getColumnName(yieldRateIndex)).setCellRenderer(cellRenderer2);
    }

    static List<String> fundCodeList;

    public static void setFundCode(List<String> fundCode) {
        FundRefreshHandler.fundCodeList = fundCode;
    }

    public void updateData(FundBean bean) {
