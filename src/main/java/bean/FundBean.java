...
public class FundBean {
    ...
    private String holdPrice;//持有价格
    private String yieldRate;//收益率
    ...
    public String getValueByColumn(String colums, boolean colorful) {
        switch (colums) {
            ...
            case "持有价格":
                return this.getHoldPrice();
            case "收益率":
                String yieldRateStr = "--";
                String yieldRate = this.getYieldRate();
                if (yieldRate != null) {
                    yieldRateStr = yieldRate.startsWith("-") ? yieldRate : "+" + yieldRate;
                }
                return yieldRateStr+ "[" + this.getJzrq() + "]";
            default:
                return "";
